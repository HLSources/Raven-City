...
{  
	...
	if ( SunEnabled == TRUE )
	{
		...
		{		
			...
			gEngfuncs.pTriAPI->SpriteTexture( (struct model_s *) gEngfuncs.GetSpritePointer(SPR_Load("sprites/lensflare1.spr")) , 0);//use hotglow, or any other sprite for the texture
			...
			gEngfuncs.pTriAPI->RenderMode(kRenderTransAdd); //additive
			gEngfuncs.pTriAPI->SpriteTexture( (struct model_s *) gEngfuncs.GetSpritePointer(SPR_Load("sprites/lensglare1.spr")) , 0);//use hotglow, or any other sprite for the texture
			gEngfuncs.pTriAPI->CullFace( TRI_NONE ); //no culling
