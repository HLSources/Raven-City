...
#define OBJ_TITLE_X		XRES(16)
#define OBJ_TITLE_Y		YRES(16)

#define OBJ_WINDOW_X				XRES(112)
#define OBJ_WINDOW_Y				YRES(80)
#define OBJ_WINDOW_SIZE_X			XRES(424)
#define OBJ_WINDOW_SIZE_Y			YRES(312)

//-----------------------------------------------------------------------------
// Purpose: Displays the MOTD and basic server information
//-----------------------------------------------------------------------------
class CObjectivesWindowPanel : public CMenuPanel
{
public:
	CObjectivesWindowPanel( const char *szObj, const char *szTitle, int iShadeFullScreen, int iRemoveMe, int x, int y, int wide, int tall );
...
//-----------------------------------------------------------------------------
// Purpose: Creates a new CObjectivesWindowPanel
// Output : CMenuPanel - interface to the panel
//-----------------------------------------------------------------------------
CMenuPanel *CObjectivesWindowPanel_Create( const char *szObj, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall )
{
	return new CObjectivesWindowPanel( szObj, szTitle, iShadeFullscreen, iRemoveMe, x, y, wide, tall );
}

//-----------------------------------------------------------------------------
// Purpose: Constructs a message panel
//-----------------------------------------------------------------------------
CObjectivesWindowPanel::CObjectivesWindowPanel( const char *szObj, const char *szTitle, int iShadeFullscreen, int iRemoveMe, int x, int y, int wide, int tall ) : CMenuPanel( iShadeFullscreen ? 100 : 255, iRemoveMe, x, y, wide, tall )
{
	...
	SchemeHandle_t hObjText = pSchemes->getSchemeHandle( "Briefing Text" );

	// color schemes
	int r, g, b, a;

	// Create the window
	m_pBackgroundPanel = new CTransparentPanel( iShadeFullscreen ? 255 : 100, OBJ_WINDOW_X, OBJ_WINDOW_Y, OBJ_WINDOW_SIZE_X, OBJ_WINDOW_SIZE_Y );
	...
	// Create the title
	Label *pLabel = new Label( "", iXPos + OBJ_TITLE_X, iYPos + OBJ_TITLE_Y );
	...
	ScrollPanel *pScrollPanel = new CTFScrollPanel( iXPos + XRES(16), iYPos + OBJ_TITLE_Y*2 + YRES(16), iXSize - XRES(32), iYSize - (YRES(48) + BUTTON_SIZE_Y*2) );
	...
	pText->setFont( pSchemes->getFont(hObjText) );
	pSchemes->getFgColor( hObjText, r, g, b, a );
	pText->setFgColor( r, g, b, a );
	pSchemes->getBgColor( hObjText, r, g, b, a );
