...
extern int R_LightPoint (vec3_t &p, vec3_t &outcolor);

struct particlevert_t
{
	vec3_t pos;
	vec3_t color;
};
...
//	if (pSys->iParticleLightCheck == NO_CHECK)
	...
	{
		vec3_t m_vColor;
		R_LightPoint( sParticle.vPosition, m_vColor );
		
		sParticle.iRed = (int)(m_vColor.x);
		sParticle.iGreen = (int)(m_vColor.y);
		sParticle.iBlue = (int)(m_vColor.z);
	}
...
bool CMappedParticle::Test( void )
{
	...
	if( gEngfuncs.pfnGetCvarFloat( "r_particle_collision" ) != 0) //If enabled, render the particles
	{
		if (pSys->iParticleCollision == PARTICLE_STUCK || 
			pSys->iParticleCollision == PARTICLE_DIE || 
			pSys->iParticleCollision == PARTICLE_BOUNCE || 
			pSys->iParticleCollision == PARTICLE_SPLASH || 
			pSys->iParticleCollision == PARTICLE_NEW_SYSTEM  ) 
		{
