...
int __MsgFunc_MotionBlur(const char *pszName, int iSize, void *pbuf )
{
	return gHUD.MsgFunc_MotionBlur( pszName, iSize, pbuf );
}
...
int __MsgFunc_Materials(const char *pszName, int iSize, void *pbuf )
{
	return gHUD.MsgFunc_Materials( pszName, iSize, pbuf );
}
// TFFree Command Menu
...
{
	if (gViewPort)
		return gViewPort->MsgFunc_TextWindow( pszName, iSize, pbuf );
	return 0;
}
...
void GL_ConDump()
{
	...
	// snip out version/vendor printout...0
	gEngfuncs.Con_Printf("----------------------------------------------------------------\n");
...
void CHud :: Init( void )
{
	...
	HOOK_MESSAGE( SetFog );
	HOOK_MESSAGE( MotionBlur );
	HOOK_MESSAGE( TextWindow );
	HOOK_MESSAGE( Materials );
	...
	BlurRate = gEngfuncs.pfnRegisterVariable( "r_blur_rate", "70", 0 );
	BlurFade = gEngfuncs.pfnRegisterVariable( "r_blur_fade", "0.1", 0 );
	BlurStrenght = gEngfuncs.pfnRegisterVariable( "r_blur_strenght", "1", 0 );
	...
	g_GrassParticleCount = gEngfuncs.pfnRegisterVariable("cl_grassamount", "100", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
	g_EnableParticle = gEngfuncs.pfnRegisterVariable("r_particle", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
	g_ParticleCollision = gEngfuncs.pfnRegisterVariable("r_particle_collision", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
	SteamFog = gEngfuncs.pfnRegisterVariable("r_steam_fog", "0", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );

	g_noblastgibs = gEngfuncs.pfnRegisterVariable("sv_noblastgibs", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
	g_flyattack = gEngfuncs.pfnRegisterVariable("sv_flyattack", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
	g_movecorpses = gEngfuncs.pfnRegisterVariable("sv_movecorpses", "1", FCVAR_CLIENTDLL | FCVAR_ARCHIVE );
...
extern int reloaded;
extern void InitSurfRender ( void );
...
//	m_hsprFont = LoadSprite("sprites/%d_font.spr");
	...
	if( pfd.cStencilBits )
	{
		gEngfuncs.Con_Printf("Stencil buffer successfully set!\n");	
	}
	else
	{
		gEngfuncs.Con_Printf("Stencil buffer initialization failure.\n");
		gEngfuncs.Con_Printf("Stencil shadows will remain disabled.\n");
	}
}
