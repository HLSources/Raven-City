...
void UTIL_GunshotDecalTrace( TraceResult *pTrace, int decalNumber )
{
	...
	if ( UTIL_PointContents(pTrace->vecEndPos) == CONTENT_SKY )
		return;
	...
	else
	{
		...
		{
			case 0: UTIL_EmitAmbientSound(ENT(0), pTrace->vecEndPos, "weapons/ric_conc1.wav", 0.25, ATTN_NORM, 0, 100); break;
			case 1: UTIL_EmitAmbientSound(ENT(0), pTrace->vecEndPos, "weapons/ric_conc2.wav", 0.25, ATTN_NORM, 0, 100); break;
		}
...
// only works in Steam
extern "C" __declspec( dllexport ) SV_SaveGameComment ( char *buffer, int max_lenght )
{
	CBaseEntity *pEntity = NULL;

	pEntity = UTIL_FindEntityByClassname( pEntity, "info_savetitle" );

	if ( pEntity )
	{
		if ( pEntity->pev->netname )
			strcpy( buffer, STRING(pEntity->pev->netname) );
		else
