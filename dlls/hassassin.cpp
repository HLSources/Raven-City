...
enum
{
	...
	SCHED_ASSASSIN_ESTABLISH_LINE_OF_FIRE,
};
...
enum
{
	TASK_ASSASSIN_CHECK_FIRE = LAST_COMMON_TASK + 1, // check if friendly is in way
	TASK_ASSASSIN_FALL_TO_GROUND, // falling and waiting to hit ground
};
...
//=========================================================
#define bits_COND_ASSASSIN_NOFIRE	( bits_COND_SPECIAL1 )
...
//=========================================================
Task_t tlAssassinEstablishLineOfFire[] = 
{
...
Schedule_t slAssassinEstablishLineOfFire[] =
{
	{ 
		tlAssassinEstablishLineOfFire,
		ARRAYSIZE ( tlAssassinEstablishLineOfFire ),
		bits_COND_NEW_ENEMY			|
...
void CHAssassin :: StartTask ( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_ASSASSIN_CHECK_FIRE:
		if ( !NoFriendlyFire() )
		{
			SetConditions( bits_COND_ASSASSIN_NOFIRE );
		}
		TaskComplete();
		break;
...
// can shoot
			...
			{
				return GetScheduleOfType ( SCHED_ASSASSIN_ESTABLISH_LINE_OF_FIRE );
			}
// new enemy
...
Schedule_t* CHAssassin :: GetScheduleOfType ( int Type ) 
{
	...
	case SCHED_ASSASSIN_ESTABLISH_LINE_OF_FIRE:
		{
			return &slAssassinEstablishLineOfFire[ 0 ];
		}
		break;
