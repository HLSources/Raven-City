...
void CItemGeneric::Spawn( void )
{
	...
	if ( m_fDisableShadows == TRUE )
		pev->renderfx = kRenderFxNoShadow;
...
//=======================
// Precipitation
// Based on code by BUzer, modified by Highlander.
//=======================
extern int gmsgRainData;
#define SF_RAIN_STARTON 1

void CPrecipitation :: KeyValue( KeyValueData *pkvd )
{
...
{
	float rainheight = pev->origin[2];
	if (gpGlobals->deathmatch && Rain_Mode == 0)
	{
		ALERT(at_console, "Rain error: only static rain in multiplayer\n");
		return; // not in multiplayer
	}
	if ( !(pev->spawnflags & SF_RAIN_STARTON) )
	{
		m_fActive = FALSE; 
	}
	else
	{
		...
		MESSAGE_BEGIN( MSG_ALL, gmsgRainData, NULL );
			WRITE_SHORT(Rain_Drips);
			WRITE_COORD(Rain_Distance);
			WRITE_COORD(Rain_windX);
			WRITE_COORD(Rain_windY);
			WRITE_COORD(Rain_randX);
			WRITE_COORD(Rain_randY);
			WRITE_SHORT(Rain_Mode);
			WRITE_COORD(rainheight);
		ALERT(at_console, "Sending enabling precipitation message\n");
		MESSAGE_END();
	}
};
void CPrecipitation::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	float rainheight = pev->origin[2];
	if (gpGlobals->deathmatch && Rain_Mode == 0)
	{
	...
	else
	{
		...
		MESSAGE_BEGIN( MSG_ALL, gmsgRainData, NULL );
			...
			WRITE_SHORT(NULL);
			WRITE_COORD(NULL);
		ALERT(at_console, "Sending disabling precipitation message\n");
		MESSAGE_END();
		return;
	}
};
LINK_ENTITY_TO_CLASS( env_precipitation, CPrecipitation );

TYPEDESCRIPTION	CPrecipitation::m_SaveData[] = 
{
	DEFINE_FIELD( CPrecipitation, m_fActive, FIELD_BOOLEAN ),
	DEFINE_FIELD( CPrecipitation, Rain_Distance, FIELD_FLOAT ),
	DEFINE_FIELD( CPrecipitation, fadeTime, FIELD_FLOAT ),
	DEFINE_FIELD( CPrecipitation, Rain_Drips, FIELD_INTEGER ),
	DEFINE_FIELD( CPrecipitation, Rain_Mode, FIELD_INTEGER ),
	DEFINE_FIELD( CPrecipitation, Rain_randX, FIELD_FLOAT ),
	DEFINE_FIELD( CPrecipitation, Rain_randY, FIELD_FLOAT ),
	DEFINE_FIELD( CPrecipitation, Rain_windX, FIELD_FLOAT ),
	DEFINE_FIELD( CPrecipitation, Rain_windY, FIELD_FLOAT ),
};
IMPLEMENT_SAVERESTORE( CPrecipitation, CBaseEntity );
//===============================================
// Motion Blur
// Entity by Highlander.
//===============================================
extern int gmsgMotionBlur;

LINK_ENTITY_TO_CLASS( env_blur, CEnvMotionBlur );
TYPEDESCRIPTION	CEnvMotionBlur::m_SaveData[] = 
{
	DEFINE_FIELD( CEnvMotionBlur, m_fBlurActive, FIELD_FLOAT ),
};
IMPLEMENT_SAVERESTORE( CEnvMotionBlur, CBaseEntity );
void CEnvMotionBlur::Spawn( void )
{
	pev->solid = SOLID_NOT;	// Remove model & collision
	pev->effects |= EF_NODRAW;
	m_fBlurActive = FALSE;
}
void CEnvMotionBlur::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	m_fBlurActive = !m_fBlurActive;

	MESSAGE_BEGIN( MSG_ALL, gmsgMotionBlur, NULL);
		WRITE_BYTE( m_fBlurActive );
	MESSAGE_END();
}
//=======================
// Sun Flare effect
// Ryokeen and Highlander.
//=======================
...
//=======================
// Radio Icon and sound
// BUzer and Highlander.
//=======================
...
LINK_ENTITY_TO_CLASS( game_radio, CRadio );
...
//=========================================================
void CRadio :: KeyValue( KeyValueData *pkvd )
{
	if (FStrEq(pkvd->szKeyName, "alternate"))
	{
		m_szTarget = ALLOC_STRING( pkvd->szValue );
		pkvd->fHandled = TRUE;
	}
	else
	{
...
void CRadio::Use( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	...
	else
	{
		if ( !FStringNull( pev->message ) && strlen( szSoundFile ) > 1 )
		{		
			EMIT_SOUND (this->edict(), CHAN_WEAPON, szSoundFile, 1, ATTN_NORM );
		}
	}
