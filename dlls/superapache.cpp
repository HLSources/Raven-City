...
class CSuperApache : public CBaseMonster
{
	int		Save( CSave &save );
...
LINK_ENTITY_TO_CLASS( monster_superapache, CSuperApache );

TYPEDESCRIPTION	CSuperApache::m_SaveData[] = 
{
	DEFINE_FIELD( CSuperApache, m_iBalls, FIELD_INTEGER ),
	DEFINE_FIELD( CSuperApache, m_flForce, FIELD_FLOAT ),
	DEFINE_FIELD( CSuperApache, m_flNextBall, FIELD_TIME ),
	DEFINE_FIELD( CSuperApache, m_vecTarget, FIELD_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_posTarget, FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_vecDesired, FIELD_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_posDesired, FIELD_POSITION_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_vecGoal, FIELD_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_angGun, FIELD_VECTOR ),
	DEFINE_FIELD( CSuperApache, m_flLastSeen, FIELD_TIME ),
	DEFINE_FIELD( CSuperApache, m_flPrevSeen, FIELD_TIME ),
//	DEFINE_FIELD( CSuperApache, m_iSoundState, FIELD_INTEGER ),		// Don't save, precached
//	DEFINE_FIELD( CSuperApache, m_iSpriteTexture, FIELD_INTEGER ),
//	DEFINE_FIELD( CSuperApache, m_iExplode, FIELD_INTEGER ),
//	DEFINE_FIELD( CSuperApache, m_iBodyGibs, FIELD_INTEGER ),
	DEFINE_FIELD( CSuperApache, m_pBeam, FIELD_CLASSPTR ),
	DEFINE_FIELD( CSuperApache, m_flGoalSpeed, FIELD_FLOAT ),
	DEFINE_FIELD( CSuperApache, m_iDoSmokePuff, FIELD_INTEGER ),
};
IMPLEMENT_SAVERESTORE( CSuperApache, CBaseMonster );


void CSuperApache :: Spawn( void )
{
	Precache( );
	// motor
	...
	pev->health			= gSkillData.apacheHealth + 10000;
...
void CSuperApache::Precache( void )
{
...
void CSuperApache::NullThink( void )
{
	StudioFrameAdvance( );
...
void CSuperApache::StartupUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
	SetThink( HuntThink );
	SetTouch( FlyTouch );
...
void CSuperApache :: Killed( entvars_t *pevAttacker, int iGib )
{
	...
	{
		m_flNextBall = gpGlobals->time + 4.0;
	}
	else
	{
		m_flNextBall = gpGlobals->time + 15.0;
	}
}

void CSuperApache :: DyingThink( void )
{
	StudioFrameAdvance( );
	pev->nextthink = gpGlobals->time + 0.1;
	...
	// still falling?
	if (m_flNextBall > gpGlobals->time )
	{
		// random explosions
...
void CSuperApache::FlyTouch( CBaseEntity *pOther )
{
...
void CSuperApache::CrashTouch( CBaseEntity *pOther )
{
	// only crash if we hit something solid
	if ( pOther->pev->solid == SOLID_BSP) 
	{
		SetTouch( NULL );
		m_flNextBall = gpGlobals->time;
...
void CSuperApache :: GibMonster( void )
{
	// EMIT_SOUND_DYN(ENT(pev), CHAN_VOICE, "common/bodysplat.wav", 0.75, ATTN_NORM, 0, 200);		
}


void CSuperApache :: HuntThink( void )
{
	StudioFrameAdvance( );
	pev->nextthink = gpGlobals->time + 0.1;
	...
	if ((m_flLastSeen + 1 > gpGlobals->time) && (m_flPrevSeen + 2 < gpGlobals->time))
	{
		...
		if (g_iSkillLevel == SKILL_EASY)
			m_flNextBall = gpGlobals->time + 10.0;
	}

	UTIL_MakeAimVectors( pev->angles );
	Vector vecEst = (gpGlobals->v_forward * 800 + pev->velocity).Normalize( );
	// ALERT( at_console, "%d %d %d %4.2f\n", pev->angles.x < 0, DotProduct( pev->velocity, gpGlobals->v_forward ) > -100, m_flNextBall < gpGlobals->time, DotProduct( m_vecTarget, vecEst ) );

	if ((m_iBalls % 2) == 1)
	{
		FireSpore( );
		m_flNextBall = gpGlobals->time + 0.5;
		if (m_iBalls <= 0)
		{
			m_flNextBall = gpGlobals->time + 10;
			m_iBalls = 10;
		}
	}
	else if (pev->angles.x < 0 && DotProduct( pev->velocity, gpGlobals->v_forward ) > -100 && m_flNextBall < gpGlobals->time)
	{
...
void CSuperApache :: FireSpore( void )
{
	...
	pBall->pev->avelocity.z = 50;

	//CSpore *pSpore = CSpore::CreateSporeRocket( vecSrc, pev->angles, this );
	//if (pSpore)
...
//		FireBullets( 1, posGun, vecGun, VECTOR_CONE_4DEGREES, 8192, BULLET_MONSTER_12MM, 1 );
		...
		CDisplacerBall *pBall = CDisplacerBall::BallCreate();
		pBall->pev->owner = edict();
		pBall->pev->origin = posGun;
		pBall->pev->angles = vecGun;
		pBall->pev->avelocity.z = 50;

		EMIT_SOUND(ENT(pev), CHAN_WEAPON, "weapons/displacer_fire.wav", 1, 0.3);
#else
		static float flNext;
		TraceResult tr;
...
void CSuperApache :: ShowDamage( void )
{
...
int CSuperApache :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType )
{
...
void CSuperApache::TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
