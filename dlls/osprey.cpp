...
void COsprey :: Spawn( void )
{
	...
	pev->flags |= EFLAG_SLERP, FL_MONSTER;
...
{
	UTIL_PrecacheOther( "monster_gangster" );
...
void COsprey :: FindAllThink( void )
{
	...
	m_iUnits = 0;
	while (m_iUnits < MAX_CARRY && (pEntity = UTIL_FindEntityByClassname( pEntity, "monster_gangster" )) != NULL)
	{
		if (pEntity->IsAlive())
		{
...
CBaseMonster *COsprey :: MakeGrunt( Vector vecSrc )
{
	...
	for (int i = 0; i < m_iUnits; i++)
	{
		if (m_hGrunt[i] == NULL || !m_hGrunt[i]->IsAlive())
		{
			...
			pEntity = Create( "monster_gangster", vecSrc, pev->angles );
			pGrunt = pEntity->MyMonsterPointer( );
			pGrunt->pev->movetype = MOVETYPE_FLY;
			pGrunt->pev->velocity = Vector( 0, 0, RANDOM_FLOAT( -196, -128 ) );
			pGrunt->SetActivity( ACT_GLIDE );
			pGrunt->pev->weapons = GANGSTER_M249;
