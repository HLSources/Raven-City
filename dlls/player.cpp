...
int gEvilImpulse101;
extern DLL_GLOBAL int		g_iSkillLevel, gDisplayTitle, g_FogEndDist, g_UpdateMaterials;
...
int gmsgPlayMP3 = 0;
int gmsgSetFog = 0;
int gmsgMotionBlur = 0;
int gmsgTimer = 0; 
int gmsgTextWindow = 0; 
int	gmsgParticles = 0;
...
void LinkUserMessages( void )
{
	...
	gmsgMotionBlur = REG_USER_MSG( "MotionBlur", -1 );
	gmsgTimer = REG_USER_MSG( "Timer", -1 ); 
	gmsgTextWindow = REG_USER_MSG( "TextWindow", -1 );
	...
	gmsgMaterials = REG_USER_MSG("Materials", -1); 
}
...
void CBasePlayer::TabulateAmmo()
{
	...
	ammo_beretta = AmmoInventory( GetAmmoIndex( "beretta" ) );
}
...
void CBasePlayer :: UpdateClientData( void )
{
	...
	if ( g_UpdateMaterials )
	{
		g_UpdateMaterials = FALSE;

		MESSAGE_BEGIN( MSG_ONE, gmsgMaterials, NULL, pev );
		MESSAGE_END();
	}
	...
	if ( m_iUpdateEffects )
	{
		...
		CBaseEntity *pPrecipitationEntity = NULL;
		pPrecipitationEntity = UTIL_FindEntityByClassname( pPrecipitationEntity, "env_precipitation" );
		CPrecipitation *pPrecipitation = (CPrecipitation *)pPrecipitationEntity;
		if ( pPrecipitationEntity && pPrecipitation->m_fActive == TRUE )
			{
				WRITE_SHORT(pPrecipitation->Rain_Drips);
				WRITE_COORD(pPrecipitation->Rain_Distance);
				WRITE_COORD(pPrecipitation->Rain_windX);
				WRITE_COORD(pPrecipitation->Rain_windY);
				WRITE_COORD(pPrecipitation->Rain_randX);
				WRITE_COORD(pPrecipitation->Rain_randY);
				WRITE_SHORT(pPrecipitation->Rain_Mode);
				WRITE_COORD(pPrecipitation->pev->origin[2]);
		...
		CBaseEntity *pSunEntity = NULL;
		pSunEntity = UTIL_FindEntityByClassname( pSunEntity, "env_sun" );
		CClientFlare *pSunFlare = (CClientFlare *)pSunEntity;
		if ( pSunFlare )
		{
			WRITE_ANGLE ( pSunFlare->m_iPitch );
			WRITE_ANGLE ( pSunFlare->m_iRoll );
			WRITE_BYTE ( TRUE );
		}
		MESSAGE_END();

		MESSAGE_BEGIN( MSG_ONE, gmsgMotionBlur, NULL, pev );
		CBaseEntity *pBlurEntity = NULL;
		pBlurEntity = UTIL_FindEntityByClassname( pBlurEntity, "env_blur" );
		CEnvMotionBlur *pBlur = (CEnvMotionBlur *)pBlurEntity;
		if ( pBlur )
		{
			WRITE_BYTE ( pBlur->m_fBlurActive );
		}
		MESSAGE_END();

		m_iUpdateEffects = FALSE;
	}
