...
Activity CBaseMonster :: GetDeathActivity ( void )
{
	...
	{
		ALERT( at_console, "> m_bitsDamageType & DMG_BLAST \n");
...
void CBaseMonster::BecomeDead( void )
{
	...
	if ( pev->enemy )
	{
		if ( !FClassnameIs( pev->enemy, "displacer_ball" ) )
			return;
	}
	...
	//check our cvar
	if ( CVAR_GET_FLOAT("sv_flyattack" ) != 0 )
	{
		if ( ( m_bitsDamageType & DMG_BLAST ) )
		{
...
int CBaseMonster :: TakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( pev->health <= 0 )
	{
		...
		if ( CVAR_GET_FLOAT("sv_noblastgibs" ) != 0 )
		{
...
int CBaseMonster :: DeadTakeDamage( entvars_t *pevInflictor, entvars_t *pevAttacker, float flDamage, int bitsDamageType )
{
	...
	if ( CVAR_GET_FLOAT("sv_movecorpses" ) != 0 )
	{
		pev->flags &= ~FL_ONGROUND;
		pev->origin.z += 1;
	...
	for (ULONG iShot = 1; iShot <= cShots; iShot++)
	{
		...
		if ( g_engfuncs.pfnPointContents( tr.vecEndPos ) != CONTENT_SKY )
		{
			...
			if (tr.flFraction != 1.0)
			{
				...
				case BULLET_MONSTER_M4A1:
					pEntity->TraceAttack(pevAttacker, gSkillData.monDmgM4a1, vecDir, &tr, DMG_BULLET);
					DecalGunshot( &tr, iBulletType, vecSrc, vecEnd );
					break;
				...
				case BULLET_MONSTER_BERETTA:		
					pEntity->TraceAttack(pevAttacker, gSkillData.monDmgBer92F, vecDir, &tr, DMG_BULLET); 
					DecalGunshot( &tr, iBulletType, vecSrc, vecEnd );
					break;
				...
				case BULLET_NONE: // FIX 
					...
					// only decal glass
					if ( !FNullEnt(tr.pHit) && VARS(tr.pHit)->rendermode != 0 && VARS(tr.pHit)->rendermode != kRenderTransAlpha )
					{
		...
		if ( g_engfuncs.pfnPointContents( tr.vecEndPos ) != CONTENT_SKY )
		{
			if (tr.flFraction != 1.0)
			{
				...
				case BULLET_PLAYER_BERETTA:		
					pEntity->TraceAttack(pevAttacker, gSkillData.plrDmgBer92F, vecDir, &tr, DMG_BULLET); 
					break;
