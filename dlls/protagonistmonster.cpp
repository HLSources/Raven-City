...
TYPEDESCRIPTION	CProtagonistMonster::m_SaveData[] = 
{
	...
	DEFINE_ARRAY( CProtagonistMonster, m_hSquadMember, FIELD_EHANDLE, MAXRC_SQUAD_MEMBERS - 1 ),
			...
			if ( !IsTalking() && HasConditions ( bits_COND_SEE_CLIENT ) && RANDOM_LONG( 0, 6 ) == 0 )
			{
				...
				if ( pPlayer )
				{
						 ...
						 UTIL_DotPoints( pPlayer->v.origin, pev->origin, gpGlobals->v_forward ) >= m_flFieldOfView )
					{
						...
						return &lPrtTlkIdleWatchClient[ 1 ];
					}

					return lPrtTlkIdleWatchClient;
				}
			}
			else
			{
...
int CProtagonistMonster :: SquadRecruit( int searchRadius, int maxMembers )
{
	...
	if ( !FStringNull( pev->netname ) )
	{
		...
		while ( pEntity )
		{
			CProtagonistMonster *pRecruit = pEntity->MyTalkSquadMonsterPointer();
