...
void DecalGunshot( TraceResult *pTrace, int iBulletType, Vector vecSrc, Vector vecEnd )
{
	...
	if ( VARS(pTrace->pHit)->solid == SOLID_BSP || VARS(pTrace->pHit)->movetype == MOVETYPE_PUSHSTEP )
	{
		...
		case BULLET_PLAYER_CROWBAR:
			// wall decal
			UTIL_DecalTrace( pTrace, DamageDecal( pEntity, DMG_BULLET, vecSrc, vecEnd ) );
			break;
...
//
// EjectLink - tosses a saw link from passed origin at passed velocity
//
void EjectLink ( const Vector &vecOrigin, const Vector &vecVelocity, float rotation, int model, int soundtype )
{
	MESSAGE_BEGIN( MSG_PVS, SVC_TEMPENTITY, vecOrigin );
		WRITE_BYTE( TE_MODEL);
		WRITE_COORD( vecOrigin.x);
...
void W_Precache(void)
{
	...
	UTIL_PrecacheOther( "monster_maria" );
	UTIL_PrecacheOther( "monster_boris" );
	UTIL_PrecacheOther( "monster_harrison" );
	...
	UTIL_PrecacheOther( "monster_superzombie" );
...
// Raven City weapons
	...
	PRECACHE_MODEL ("sprites/debris/debris_concrete1.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_concrete2.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_concrete3.spr");    

	PRECACHE_MODEL ("sprites/debris/debris_dirt1.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_dirt2.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_dirt3.spr");    


	PRECACHE_MODEL ("sprites/debris/debris_snow1.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_snow2.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_snow3.spr");    

	PRECACHE_MODEL ("sprites/debris/debris_tile1.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_tile2.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_tile3.spr");    

	PRECACHE_MODEL ("sprites/debris/debris_wood1.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_wood2.spr");    
	PRECACHE_MODEL ("sprites/debris/debris_wood3.spr");    
...
BOOL CBasePlayerWeapon :: DefaultReload( int iClipSize, int iAnim, float fDelay, int body, int override )
{
...
BOOL CBasePlayerWeapon :: PlayDispEmptySound( void )
{
	if (m_iPlayEmptySound)
	{
		EMIT_SOUND(ENT(m_pPlayer->pev), CHAN_WEAPON, "buttons/button11.wav", 0.8, ATTN_NORM);
		m_iPlayDispEmptySound = 0;
		return 0;
	}
    	return 0;
}

void CBasePlayerWeapon :: ResetDispEmptySound( void )
{
	m_iPlayDispEmptySound = 1;
}
...
//=========================================================
// WeaponBox - Pack player's weapons into a box.
//=========================================================
...
void CWeaponBox::Touch( CBaseEntity *pOther )
{
	if (!(pev->flags & FL_ONGROUND))
	{
		...
		if ( m_cWeaponType == 1 )
		{
			...
			{
			case 0: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "debris/metal1.wav", 1, ATTN_NORM, 0, pitch); break;
			case 1: EMIT_SOUND_DYN( ENT(pev), CHAN_VOICE, "debris/metal3.wav", 1, ATTN_NORM, 0, pitch); break;
			}
		}
		else
		{
...
BOOL CWeaponBox::PackWeapon( CBasePlayerItem *pWeapon )
{
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_displacer")))
	{
		m_cWeaponType = 0;
		SET_MODEL( ENT(pev), "models/w_displacer.mdl");
	}
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_sniperrifle")))
	{
		m_cWeaponType = 0;
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_knife")))
	{
		m_cWeaponType = 1;
		SET_MODEL( ENT(pev), "models/w_knife.mdl");
	}
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_sporelauncher")))
	{
		m_cWeaponType = 0;
		SET_MODEL( ENT(pev), "models/w_spore_launcher.mdl");
	}
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_shockrifle")))
	{
		m_cWeaponType = 0;
		SET_MODEL( ENT(pev), "models/w_shock_rifle.mdl");
	}
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_pipewrench")))
	{
		m_cWeaponType = 1;
	...
	else if ((!strcmp((char *)STRING( pWeapon->pev->classname ), "weapon_beretta")))
	{
		m_cWeaponType = 0;
...
{
	DEFINE_FIELD( CGlock, m_iSilencer, FIELD_INTEGER ),
};
