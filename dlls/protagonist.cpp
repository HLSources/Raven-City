...
//																					 //
// Purpose:	AI for Protagonist monsters Maria and Boris								 //
//																					 //
// $NoKeywords: $																	 //
//===================================================================================//
...
//=========================================================
float	CProtagonistMonster::g_talkWaitTime = 0;		// time delay until it's ok to speak: used so that two NPCs don't talk at once

// NOTE: m_voicePitch & m_szGrp should be fixed up by precache each save/restore

TYPEDESCRIPTION	CProtagonistMonster::m_SaveData[] = 
{
	DEFINE_FIELD( CProtagonistMonster, m_bitsSaid, FIELD_INTEGER ),
	DEFINE_FIELD( CProtagonistMonster, m_nSpeak, FIELD_INTEGER ),
	DEFINE_FIELD( CProtagonistMonster, m_useTime, FIELD_TIME ),
	DEFINE_FIELD( CProtagonistMonster, m_iszUse, FIELD_STRING ),
	DEFINE_FIELD( CProtagonistMonster, m_iszUnUse, FIELD_STRING ),
	DEFINE_FIELD( CProtagonistMonster, m_flLastSaidSmelled, FIELD_TIME ),
	DEFINE_FIELD( CProtagonistMonster, m_flStopTalkTime, FIELD_TIME ),
	DEFINE_FIELD( CProtagonistMonster, m_hTalkTarget, FIELD_EHANDLE ),
	DEFINE_FIELD( CProtagonistMonster, m_hSquadLeader, FIELD_EHANDLE ),
	DEFINE_ARRAY( CProtagonistMonster, m_hSquadMember, FIELD_EHANDLE, MAXPROT_SQUAD_MEMBERS - 1 ),
	DEFINE_FIELD( CProtagonistMonster, m_fEnemyEluded, FIELD_BOOLEAN ),
	DEFINE_FIELD( CProtagonistMonster, m_flLastEnemySightTime, FIELD_TIME ),
	DEFINE_FIELD( CProtagonistMonster, m_hHealTarget, FIELD_EHANDLE ),

	DEFINE_FIELD( CProtagonistMonster, m_iMySlot, FIELD_INTEGER ),
};

IMPLEMENT_SAVERESTORE( CProtagonistMonster, CBaseMonster );

// array of friend names
char *CProtagonistMonster::m_szPFriends[PRT_CFRIENDS] = 
{
	"monster_boris",
	"monster_maria",
...
Task_t	tlPrtIdleResponse[] =
{
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE	},// Stop and listen
	{ TASK_WAIT,			(float)0.5		},// Wait until sure it's me they are talking to
	{ TASK_PRT_EYECONTACT,	(float)0		},// Wait until speaker is done
	{ TASK_PRT_RESPOND,		(float)0		},// Wait and then say my response
	{ TASK_PRT_IDEALYAW,	(float)0		},// look at who I'm talking to
	{ TASK_FACE_IDEAL,		(float)0		}, 
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE	},
	{ TASK_PRT_EYECONTACT,	(float)0		},// Wait until speaker is done
};

Schedule_t	slPrtIdleResponse[] =
{
	{ 
		tlPrtIdleResponse,
		ARRAYSIZE ( tlPrtIdleResponse ), 
		bits_COND_NEW_ENEMY		|
...
Task_t	tlPrtIdleSpeak[] =
{
	{ TASK_PRT_SPEAK,		(float)0		},// question or remark
	{ TASK_PRT_IDEALYAW,	(float)0		},// look at who I'm talking to
	{ TASK_FACE_IDEAL,		(float)0		}, 
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE	},
	{ TASK_PRT_EYECONTACT,	(float)0		},
	{ TASK_WAIT_RANDOM,		(float)0.5		},
};

Schedule_t	slPrtIdleSpeak[] =
{
	{ 
		tlPrtIdleSpeak,
		ARRAYSIZE ( tlPrtIdleSpeak ), 
		bits_COND_NEW_ENEMY		|
...
Schedule_t	slPrtIdleSpeakWait[] =
{
	{ 
		tlPrtIdleSpeakWait,
		ARRAYSIZE ( tlPrtIdleSpeakWait ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_CLIENT_PUSH	|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_MEDIC_HEAL	|
...
Task_t	tlPrtIdleHello[] =
{
	{ TASK_SET_ACTIVITY,	(float)ACT_IDLE	},// Stop and talk
	{ TASK_PRT_HELLO,		(float)0		},// Try to say hello to player
	{ TASK_PRT_EYECONTACT,	(float)0		},
...
Schedule_t	slPrtIdleHello[] =
{
	{ 
		tlPrtIdleHello,
		ARRAYSIZE ( tlPrtIdleHello ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_CLIENT_PUSH	|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
...
Task_t	tlPrtIdleStopShooting[] =
{
	{ TASK_PRT_STOPSHOOTING,	(float)0		},// tell player to stop shooting friend
};

Schedule_t	slPrtIdleStopShooting[] =
{
	{ 
		tlPrtIdleStopShooting,
		ARRAYSIZE ( tlPrtIdleStopShooting ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_MEDIC_HEAL	|
...
Schedule_t	slPrtMoveAway[] =
{
	{
		tlPrtMoveAway,
		ARRAYSIZE ( tlPrtMoveAway ),
		0,
		0,
		"MoveAway"
	},
...
Task_t	tlPrtMoveAwayFail[] =
{
	{ TASK_STOP_MOVING,				(float)0		},
	{ TASK_FACE_PLAYER,				(float)0.5		},
};

Schedule_t	slPrtMoveAwayFail[] =
{
	{
		tlPrtMoveAwayFail,
		ARRAYSIZE ( tlPrtMoveAwayFail ),
		0,
		0,
		"MoveAwayFail"
	},
...
Schedule_t	slPrtMoveAwayFollow[] =
{
	{
		tlPrtMoveAwayFollow,
		ARRAYSIZE ( tlPrtMoveAwayFollow ),
		0,
		0,
		"MoveAwayFollow"
	},
...
Task_t	tlPrtTlkIdleWatchClient[] =
{
	{ TASK_STOP_MOVING,			0				},
	{ TASK_SET_ACTIVITY,		(float)ACT_IDLE	},
	{ TASK_PRT_LOOK_AT_CLIENT,	(float)6		},
};

Task_t	tlPrtTlkIdleWatchClientStare[] =
{
	{ TASK_STOP_MOVING,			0				},
	{ TASK_SET_ACTIVITY,		(float)ACT_IDLE	},
	{ TASK_PRT_CLIENT_STARE,	(float)6		},
	{ TASK_PRT_STARE,			(float)0		},
	{ TASK_PRT_IDEALYAW,		(float)0		},// look at who I'm talking to
	{ TASK_FACE_IDEAL,			(float)0		}, 
	{ TASK_SET_ACTIVITY,		(float)ACT_IDLE	},
	{ TASK_PRT_EYECONTACT,		(float)0		},
...
Schedule_t	slPrtTlkIdleWatchClient[] =
{
	{ 
		tlPrtTlkIdleWatchClient,
		ARRAYSIZE ( tlPrtTlkIdleWatchClient ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
	...
	{ 
		tlPrtTlkIdleWatchClientStare,
		ARRAYSIZE ( tlPrtTlkIdleWatchClientStare ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_HEAVY_DAMAGE	|
		bits_COND_HEAR_SOUND	|
...
Schedule_t	slPrtTlkIdleEyecontact[] =
{
	{ 
		tlPrtTlkIdleEyecontact,
		ARRAYSIZE ( tlPrtTlkIdleEyecontact ), 
		bits_COND_NEW_ENEMY		|
		bits_COND_CLIENT_PUSH	|
		bits_COND_LIGHT_DAMAGE	|
		bits_COND_MEDIC_HEAL	|
...
DEFINE_CUSTOM_SCHEDULES( CProtagonistMonster )
{
	slPrtIdleResponse,
	slPrtIdleSpeak,
	slPrtIdleHello,
	slPrtIdleSpeakWait,
	slPrtIdleStopShooting,
	slPrtMoveAway,
	slPrtMoveAwayFollow,
	slPrtMoveAwayFail,
	slPrtTlkIdleWatchClient,
	&slPrtTlkIdleWatchClient[ 1 ],
	slPrtTlkIdleEyecontact,
};

IMPLEMENT_CUSTOM_SCHEDULES( CProtagonistMonster, CBaseMonster );


void CProtagonistMonster :: SetActivity ( Activity newActivity )
{
...
void CProtagonistMonster :: StartTask( Task_t *pTask )
{
	switch ( pTask->iTask )
	{
	case TASK_PRT_SPEAK:
	...
	case TASK_PRT_LOOK_AT_CLIENT:
	case TASK_PRT_CLIENT_STARE:
		// track head to the client for a while.
		m_flWaitFinished = gpGlobals->time + pTask->flData;
		break;

	case TASK_PRT_EYECONTACT:
		break;

	case TASK_PRT_IDEALYAW:
	...
	case TASK_PRT_HEADRESET:
		// reset head position after looking at something
		m_hTalkTarget = NULL;
		TaskComplete();
		break;

	case TASK_PRT_STOPSHOOTING:
...
void CProtagonistMonster :: RunTask( Task_t *pTask )
{
	switch( pTask->iTask )
	{
	case TASK_PRT_CLIENT_STARE:
	case TASK_PRT_LOOK_AT_CLIENT:
		...
		if ( pTask->iTask == TASK_PRT_CLIENT_STARE )
		{
...
void CProtagonistMonster :: Killed( entvars_t *pevAttacker, int iGib )
{
...
CBaseEntity	*CProtagonistMonster::EnumFriends( CBaseEntity *pPrevious, int listNumber, BOOL bTrace )
{
	...
	pszFriend = m_szPFriends[ FriendNumber(listNumber) ];
...
void CProtagonistMonster::AlertFriends( void )
{
	CBaseEntity *pFriend = NULL;
	int i;

	// for each friend in this bsp...
	for ( i = 0; i < PRT_CFRIENDS; i++ )
	{
...
void CProtagonistMonster::ShutUpFriends( void )
{
	CBaseEntity *pFriend = NULL;
	int i;
...
float CProtagonistMonster::TargetDistance( void )
{
...
//=========================================================
void CProtagonistMonster :: HandleAnimEvent( MonsterEvent_t *pEvent )
{
	switch( pEvent->event )
	{		
...
// monsters derived from CProtagonistMonster should call this in precache()

void CProtagonistMonster :: TalkInit( void )
{
	// every new talking monster must reset this global, otherwise
	// when a level is loaded, nobody will talk (time is reset to 0)

	CProtagonistMonster::g_talkWaitTime = 0;
...
// nearest player
//=========================================================
CBaseEntity *CProtagonistMonster :: FindNearestFriend(BOOL fPlayer)
{
	CBaseEntity *pFriend = NULL;
	...
	for (i = cfriends-1; i > -1; i--)
	{
		if (fPlayer)
			...
			pszFriend = m_szPFriends[FriendNumber(i)];
...
int CProtagonistMonster :: GetVoicePitch( void )
{
	return m_voicePitch + RANDOM_LONG(0,3);
}


void CProtagonistMonster :: Touch( CBaseEntity *pOther )
{
...
//=========================================================
void CProtagonistMonster :: IdleRespond( void )
{
...
int CProtagonistMonster :: FOkToSpeak( void )
{
...
int CProtagonistMonster::CanPlaySentence( BOOL fDisregardState ) 
{ 
	if ( fDisregardState )
...
// FIdleStare
//=========================================================
int CProtagonistMonster :: FIdleStare( void )
{
	if (!FOkToSpeak())
		return FALSE;
...
//=========================================================
int CProtagonistMonster :: FIdleHello( void )
{
	if (!FOkToSpeak())
		return FALSE;
...
void CProtagonistMonster :: IdleHeadTurn( Vector &vecFriend )
{
...
//=========================================================
int CProtagonistMonster :: FIdleSpeak ( void )
{ 
	...
	if (pFriend && !(pFriend->IsMoving()) && (RANDOM_LONG(0,99) < 75))
	{
		...
		CProtagonistMonster *pTalkMonster = (CProtagonistMonster *)pFriend;
...
void CProtagonistMonster::PlayScriptedSentence( const char *pszSentence, float duration, float volume, float attenuation, BOOL bConcurrent, CBaseEntity *pListener )
{
	if ( !bConcurrent )
		ShutUpFriends();
...
void CProtagonistMonster::PlaySentence( const char *pszSentence, float duration, float volume, float attenuation )
{
	if ( !pszSentence )
		return;
	...
	CProtagonistMonster::g_talkWaitTime = gpGlobals->time + duration + 2.0;
	if ( pszSentence[0] == '!' )
...
// talking.
//=========================================================
void CProtagonistMonster :: Talk( float flDuration )
{
	if ( flDuration <= 0 )
	{
		// no duration :( 
...
void CProtagonistMonster :: SetAnswerQuestion( CProtagonistMonster *pSpeaker )
{
	if ( !m_pCine )
		ChangeSchedule( slPrtIdleResponse );
	m_hTalkTarget = (CBaseMonster *)pSpeaker;
}

int CProtagonistMonster :: TakeDamage( entvars_t* pevInflictor, entvars_t* pevAttacker, float flDamage, int bitsDamageType)
{
	if ( IsAlive() )
	{
		...
		if (pevAttacker && m_MonsterState != MONSTERSTATE_PRONE && FBitSet(pevAttacker->flags, FL_CLIENT))
		{
			...
			if (pFriend && pFriend->IsAlive())
			{
				...
				CProtagonistMonster *pTalkMonster = (CProtagonistMonster *)pFriend;
				pTalkMonster->ChangeSchedule( slPrtIdleStopShooting );
			}
		}
	}
...
//=========================================================
void CProtagonistMonster :: TraceAttack( entvars_t *pevAttacker, float flDamage, Vector vecDir, TraceResult *ptr, int bitsDamageType)
{
	// I cant understand this! These monsters get more damage than the male assassin and the others.
...
Schedule_t* CProtagonistMonster :: GetScheduleOfType ( int Type )
{
	switch( Type )
	{
	case SCHED_MOVE_AWAY:
		return slPrtMoveAway;

	case SCHED_MOVE_AWAY_FOLLOW:
		return slPrtMoveAwayFollow;

	case SCHED_MOVE_AWAY_FAIL:
		return slPrtMoveAwayFail;

	case SCHED_TARGET_FACE:
		// speak during 'use'
		if (RANDOM_LONG(0,99) < 2)
			//ALERT ( at_console, "target chase speak\n" );
			return slPrtIdleSpeakWait;
		else
			return slIdleStand;
	...
	case SCHED_IDLE_STAND:
		{	
			...
			if (!FBitSet(m_bitsSaid, bit_saidWoundLight) && (pev->health <= (pev->max_health * 0.75)))
			{
				...
				//CProtagonistMonster::g_talkWaitTime = gpGlobals->time + RANDOM_FLOAT(2.8, 3.2);
			...
			if ( !IsTalking() && HasConditions ( bits_COND_SEE_CLIENT ) && RANDOM_LONG( 0, 6 ) == 0 )
			{
				...
				if ( pPlayer )
				{
						 ...
						 UTIL_DotPoints( pPlayer->v.origin, pev->origin, gpGlobals->v_forward ) >= m_flFieldOfView )
					{
						...
						return &slPrtTlkIdleWatchClient[ 1 ];
					}

					return slPrtTlkIdleWatchClient;
				}
			}
			else
			{
			...
			// NOTE - caller must first CProtagonistMonster::GetScheduleOfType, 
...
//=========================================================
BOOL CProtagonistMonster :: IsTalking( void )
{
...
//=========================================================
void CProtagonistMonster :: PrescheduleThink ( void )
{
...
void CProtagonistMonster :: TrySmellTalk( void )
{
	if ( !FOkToSpeak() )
		return;
...
int CProtagonistMonster::IRelationship( CBaseEntity *pTarget )
{
	if ( pTarget->IsPlayer() )
		if ( m_afMemory & bits_MEMORY_PROVOKED )
			return R_HT;
...
void CProtagonistMonster::StopFollowing( BOOL clearSchedule )
{
	if ( IsFollowing() )
	{
		if ( !(m_afMemory & bits_MEMORY_PROVOKED) )
...
void CProtagonistMonster::StartFollowing( CBaseEntity *pLeader )
{
	if ( m_pCine )
...
BOOL CProtagonistMonster::CanFollow( void )
{
...
void CProtagonistMonster :: FollowerUse( CBaseEntity *pActivator, CBaseEntity *pCaller, USE_TYPE useType, float value )
{
...
void CProtagonistMonster :: GruntHealerCall( CBaseEntity *pGrunt )
{
	// Don't allow use during a scripted_sentence
	if ( m_useTime > gpGlobals->time )
		return;
	...
	//	WRITE_COORD( 250 );	// radius
	//	WRITE_BYTE( 255 );	// R
	//	WRITE_BYTE( 192 );	// G
	//	WRITE_BYTE( 64 );	// B
	//	WRITE_BYTE( 999999998 );	// life * 10
...
void CProtagonistMonster::KeyValue( KeyValueData *pkvd )
{
...
void CProtagonistMonster::Precache( void )
{
	if ( m_iszUse )
...
//=========================================================
BOOL CProtagonistMonster :: OccupySlot( int iDesiredSlots )
{
	int i;
	int iMask;
	int iSquadSlots;
	...
	CProtagonistMonster *pSquadLeader = MySquadLeader();
...
// VacateSlot 
//=========================================================
void CProtagonistMonster :: VacateSlot()
{
...
// ScheduleChange
//=========================================================
void CProtagonistMonster :: ScheduleChange ( void )
{
	VacateSlot();
}

// These functions are still awaiting conversion to CProtagonistMonster 
...
//
//=========================================================
void CProtagonistMonster :: SquadRemove( CProtagonistMonster *pRemove )
{
	ASSERT( pRemove!=NULL );
	...
	{
		for (int i = 0; i < MAXPROT_SQUAD_MEMBERS-1;i++)
		{
			CProtagonistMonster *pMember = MySquadMember(i);
			if (pMember)
			{
...
//
//=========================================================
BOOL CProtagonistMonster :: SquadAdd( CProtagonistMonster *pAdd )
{
	ASSERT( pAdd!=NULL );
	ASSERT( !pAdd->InSquad() );
	ASSERT( this->IsLeader() );
...
//
//=========================================================
void CProtagonistMonster :: SquadPasteEnemyInfo ( void )
{
	CProtagonistMonster *pSquadLeader = MySquadLeader( );
	if (pSquadLeader)
...
//
//=========================================================
void CProtagonistMonster :: SquadCopyEnemyInfo ( void )
{
	CProtagonistMonster *pSquadLeader = MySquadLeader( );
	if (pSquadLeader)
...
//
//=========================================================
void CProtagonistMonster :: SquadMakeEnemy ( CBaseEntity *pEnemy )
{
	if (!InSquad())
		return;
	...
	CProtagonistMonster *pSquadLeader = MySquadLeader( );
	for (int i = 0; i < MAXPROT_SQUAD_MEMBERS; i++)
	{
		CProtagonistMonster *pMember = pSquadLeader->MySquadMember(i);
		if (pMember)
		{
...
//
//=========================================================
int CProtagonistMonster :: SquadCount( void )
{
	if (!InSquad())
		return 0;
...
//
//=========================================================
int CProtagonistMonster :: SquadRecruit( int searchRadius, int maxMembers )
{
	int squadCount;
	...
	if ( !FStringNull( pev->netname ) )
	{
		...
		while ( pEntity )
		{
			CProtagonistMonster *pRecruit = pEntity->MyProtagonistMonsterPointer();
...
// CheckEnemy
//=========================================================
int CProtagonistMonster :: CheckEnemy ( CBaseEntity *pEnemy )
{
	int iUpdatedLKP;
...
// StartMonster
//=========================================================
void CProtagonistMonster :: StartMonster( void )
{
	...
	if ( ( m_afCapability & bits_CAP_SQUAD ) && !InSquad() )
	{
		if ( !FStringNull( pev->netname ) )
		{
			...
			if ( !( pev->spawnflags & SF_PROTAGONIST_LEADER ) )
			{
				return;
			}
		}
...
//=========================================================
BOOL CProtagonistMonster :: NoFriendlyFire( void )
{
	if ( !InSquad() )
	{
		return TRUE;
...
//=========================================================
MONSTERSTATE CProtagonistMonster :: GetIdealState ( void )
{
	int	iConditions;
...
//=========================================================
BOOL CProtagonistMonster :: FValidateCover ( const Vector &vecCoverLocation )
{
	if ( !InSquad() )
	{
		return TRUE;
...
//=========================================================
BOOL CProtagonistMonster :: SquadEnemySplit ( void )
{
	if (!InSquad())
		return FALSE;
...
// on proximity to others in the squad)
//=========================================================
BOOL CProtagonistMonster :: SquadMemberInRange ( const Vector &vecLocation, float flDist )
{
	if (!InSquad())
		return FALSE;
	...
	for (int i = 0; i < MAXPROT_SQUAD_MEMBERS; i++)
	{
		CProtagonistMonster *pSquadMember = pSquadLeader->MySquadMember(i);
