...
#define CHAR_TEX_RUG		'R'
#define CHAR_TEX_ORGANIC	'N'
#define CHAR_TEX_GRAVEL		'A'
...
#define STEP_RUG		11		// rug
#define STEP_WOOD		12		// wood
#define STEP_GRAVEL		13		// gravel
...
//		return;
	...
	strcpy( szFile, levelname );
	ch = strchr( szFile, '.' );
	*ch = '\0';
	strcat( szFile, "_materials.txt" );

	fileSize = pmove->COM_FileSize( szFile );
	pMemFile = pmove->COM_LoadFile( szFile, 5, NULL );
...
void PM_PlayStepSound( int step, float fvol )
{
	...
	case STEP_RUG:
		...
		{
		// right foot
		case 0:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_rug1.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 1:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_rug3.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		// left foot
		case 2:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_rug2.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		case 3:	pmove->PM_PlaySound( CHAN_BODY, "player/pl_rug4.wav", fvol, ATTN_NORM, 0, PITCH_NORM );	break;
		}
		break;
	case STEP_GRAVEL:
		switch(irand)
