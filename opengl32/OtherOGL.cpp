...
//																										//
// Purpose: Functions not mainly used by half-life, or functions that would allow limited to no			//
...
const GLubyte * sys_glGetString (GLenum name)
{
	if (name == 0x1F03) // GL_EXTENSIONS
	{
		...
		strcat(extstring, "RAVEN_HACKS_V1");
...
int sys_wglChoosePixelFormat(HDC hdc, PIXELFORMATDESCRIPTOR *pfd)
{
	...
	pfd->cColorBits = 32; // Set color bits.
	pfd->cDepthBits = 24; // Set depth bits.
	pfd->cStencilBits = 4; // Set stencil bits.
